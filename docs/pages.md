...
- **Taxonomy bảo dưỡng = 2 trục**: NHÓM (vật, `bao_duong.nhom`) + LOẠI (hành động, `bao_duong.loai`). Driver CHỈ có `loai ∈ {sua, thay_moi}` (KHÔNG `giay_to`). Modal có `#maint-nhom` (5 nhóm: Phanh/Lốp/Động cơ/Điện/Khác, placeholder rỗng) đặt trước `#maint-bophan`; `PARTS_BY_NHOM`/`NHOM_KEYS` duplicate-local (không shared.js). `filterDriverBoPhan()` đổ `#maint-bophan-list` theo nhóm đang chọn (rỗng nếu chưa chọn) — là writer duy nhất của datalist; gọi mỗi lần `openMaintenanceModal` (reset nhom='', loai='sua') và sau khi AI autofill. `submitMaintenance`: `nhom` bắt buộc (toast "Vui lòng chọn nhóm"), `bo_phan` bắt buộc, cả hai vào INSERT
- Modal bảo dưỡng có nút "✨ Tự điền loại & bộ phận" (`#btn-maint-ai`) → `autoFillMaintLoai()`: đọc `#maint-mota`, `POST /api/phan-loai-bao-duong` → đọc `{loai, nhom, bo_phan}`: set `#maint-loai` (nếu loai ∈ `{sua, thay_moi}`), set `#maint-nhom` + gọi `filterDriverBoPhan()` (nếu nhom ∈ `NHOM_KEYS`), rồi set `#maint-bophan` (nếu non-rỗng). Disable nút + đổi text "Đang xử lý..." trong try/finally; lỗi/`!r.ok`/network → toast nhẹ "Không gợi ý được, chọn tay nhé", KHÔNG chặn. Tài xế review tay, không validate thêm client.
...
**Bảo dưỡng**
- **Taxonomy 2 trục**: NHÓM (`bao_duong.nhom`) + LOẠI (`bao_duong.loai`). Owner/supervisor có đủ 3 `loai`: `maintenanceTypes = [['sua','Sửa'],['thay_moi','Thay mới'],['giay_to','Giấy tờ']]`. `PARTS_BY_NHOM`/`NHOM_KEYS` duplicate-local. `LEGACY_LOAI_LABELS` = nhãn cho record cũ (`hong_hoc/linh_kien/lop_xe/dinh_ky` → "... (cũ)"); KHÔNG migrate data cũ
- Modal (thứ tự field): Ngày → **Loại** (`#maint-modal-loai`, onchange `applyLoaiVisibility()`) → Nhóm (`#row-maint-nhom` > `#maint-modal-nhom`, onchange `filterModalBoPhan()`) → Bộ phận (`#row-maint-bophan`) → Hình thức sửa (`#row-maint-hinhthuc`) → Mô tả → ...
- `applyLoaiVisibility()`: `loai==='giay_to'` → `display:none` 3 hàng nhóm/bộ phận/hình-thức + clear value 3 control; ngược lại `display:''`. Vì set `.value` lập trình KHÔNG fire onchange, `openMaintenanceModal` phải **gọi tường minh** `applyLoaiVisibility()` (gỡ ẩn dính từ lần `giay_to` trước) rồi `filterModalBoPhan()`
- `filterModalBoPhan()`: đổ `#maint-modal-bophan-list` theo nhóm đang chọn (rỗng nếu `''`) — **writer duy nhất** của datalist này. `loadMaintenanceModal()` KHÔNG còn populate datalist; gọi sau reset nhom trong `addMaintenanceModal` để tránh suggestion stale
- `addMaintenanceModal()`: `loai==='giay_to'` → ép `nhom=null, bo_phan=null, hinh_thuc_sua=null`, BỎ validate nhóm + hình-thức. Ngược lại: `nhom` bắt buộc (toast "Vui lòng chọn nhóm"), `hinh_thuc_sua` bắt buộc. `nhom` vào INSERT
- `loadMaintenanceModal()`: trước bảng lịch sử phẳng, render `summaryContainer` "📊 Tổng hợp theo bộ phận" — rollup group theo `bo_phan` (null → nhóm "Khác"; record `giay_to` có `bo_phan=null` rơi vào đây — **Phần B sẽ đổi rollup sang gom theo NHÓM**), mỗi nhóm `{soLan, lanGanNhat, tongChiPhi, cacNgay[]}`, sort `soLan` desc. Date parse thủ công từ `YYYY-MM-DD` (KHÔNG dùng `formatDate` để né bug timezone của `date` column). Badge cảnh báo `⚠️ thay lại sau N ngày` (màu `--warning`) khi nhóm có `soLan >= 2` và khoảng cách giữa 2 lần gần nhất `> 0 && < NGUONG_THAY_LAI` (30 ngày). Badge chỉ hiển thị, KHÔNG chặn/kết luận
- Bảng history join: `.select('*, tai_xe:users!tai_xe_id(full_name)')` — "Người nhập": `'driver'` → `👤 {full_name}`, `'owner'` → `🏢 Chủ xe`. Cột Loại: `loaiLabels[m.loai] || LEGACY_LOAI_LABELS[m.loai] || m.loai || '-'`
- Cột Mô tả append `→ Xem chuyến` (mở tab mới) nếu `trip_id` có giá trị
- Cột Mô tả render badge hình thức sửa inline (🏭 Tại gara nền `#e8f5ed` / 🔧 Lưu động nền `#fdf0e3`); `null`/giá trị lạ không render. **Gotcha**: reset pattern 2 form bảo dưỡng bất đối xứng — `driver-page.html` reset on-open (`openMaintenanceModal`), `vehicles.html` reset on-success (`addMaintenanceModal`); cả hai đều gọi `filter*BoPhan()` sau reset nhom để datalist không stale
